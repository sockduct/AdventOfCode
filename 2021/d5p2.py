...
def main(verbose=False):
    ...
    for line in lines:
        if line.ishor():
            ...
            if verbose:
                print(f'Processing diagonal (45°) line {line}')
            if line.x2 >= line.x1:
            ...
            if y2 >= y1:
                for xpos, ypos in zip(range(x1, x2 + 1), range(y1, y2 + 1)):
                    matrix.inc(xpos, ypos)
            else:
                for xpos, ypos in zip(range(x1, x2 + 1), range(y1, y2 - 1, -1)):
                    matrix.inc(xpos, ypos)
        elif verbose:
